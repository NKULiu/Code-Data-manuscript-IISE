                   ...
                   .packages= c("MASS","glmnet",'pracma','ks',
                                'foreach','doParallel','mvtnorm'))%dopar%{
                                  ...
                                  if(SDA0){
                                    ...
                                    Method1<-c(Method1,sapply(alpha, function(x){paste0('SDA0-',x)}))
                                  }
                                  if(MOW){
                                    k1=k1+1
                                    res<-Func_MOW(X2,op,op1,det1,det2,sig2,alpha)
                                    Allres$RES1[[k1]]<-t(sapply(1:length(alpha),function(x){Func_Measure(res$Res[[x]],Ind,p)}))
                                    Method1<-c(Method1,sapply(alpha, function(x){paste0('MOW-',x)}))
                                  }
                                  if(LEB0){
                                    k1=k1+1
                                  ...
                                  if(SDA){
                                    k1=k1+1
                                    Det <- Func_SDA2(X1, X2,alpha=alpha, model ='mean')
                                    Allres$RES1[[k1]]<-t(sapply(1:length(alpha),function(x){Func_Measure(Det$out[[x]],Ind,p)}))
                                  ...
                                  if(MDW){
                                    ...
                                    Allres$RES1[[k1]]<-t(sapply(1:length(alpha),function(x){Func_Measure(res$Res[[x]],Ind,p)}))
                                    Method1<-c(Method1,sapply(alpha, function(x){paste0('MDW-',x)}))
                                  }
                                  if(LEB){
                                    k1=k1+1
...
proc.time() - tic
df.temp1
#write.csv(df.temp1,file = 'Res_Table1.csv')
